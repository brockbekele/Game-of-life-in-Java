...
import spm.GameOfLifeProject.Interval;

import java.util.Random;

import java.util.concurrent.Future;

/** The GolSkandium class which represent the skandium parallel implementation
 * 
 * @author biruk
 *
 */
...
public class GolSkandium {
	 ...
	 * 
	 * @param matrix_size equals to the matrix size
	 */
	...
	public static void generate() {
		...
		try {
			...
			for (int i = 1; i < Matrix.matrix.length; i++) {
				Matrix.matrix[i][Matrix.matrix.length - 1] = Matrix.matrix[i][1]; // rightmost ghost colum
				Matrix.matrix[i][0] = Matrix.matrix[i][Matrix.matrix.length - 2]; // leftmost ghost coumn
			}
		...
		public static void generateseed(long seed) {
			...
			try {
				...
				// Use for the ghost column
				for (int i = 1; i < Matrix.matrix.length; i++) {
					Matrix.matrix[i][Matrix.matrix.length - 1] = Matrix.matrix[i][1]; // rightmost ghost column
