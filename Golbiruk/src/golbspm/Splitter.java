 ...
 */
    ...
    public Interval[] split(Interval arg0) throws Exception {
        ...
        int rem = Golskandium.MATRIX_SIZE % threads;//keep the reminder after equal distribution of rows 
        Interval[] result = new Interval[threads];


        int rowStart = arg0.a;//1
        for (int i = 0; i < threads; i++) {
            rowLength = (i < rem) ? rowSize + 1 : rowSize;//the reminder row is added to each, from the begning
